...
export class GridRenderer {
  ...
  pickDirection(availableDirections: Direction[] = ['down', 'left', 'right', 'up']): Direction {
  ...
  getClosestIntersectionCoordinates(event: MouseEvent): [number, number] {
    ...
    const x = Math.round(event.offsetX / squareLength) * squareLength * this.pixelRatio;
    const y = Math.round(event.offsetY / squareLength) * squareLength * this.pixelRatio;
    return [x, y];
  }
  ...
  limitLineLength(line: Line): void {
    function calculateSum(): number {
      return line.coordinates.reduce((acc, [x, y], index, arr) => {
        ...
        acc += Math.sqrt(Math.pow(x - arr[index - 1][0], 2) + Math.pow(y - arr[index - 1][1], 2));
        return acc;
      }, 0);
    }
  ...
  getNewCoordinates(line: Line, step: number = this.speed): void {
    ...
    const handleLineIntersectingNode = (currentDirection: Direction, oppositeDirection: Direction) => {
      ...
      line.currentDirection = this.pickDirection(
        allowedDirections[initialDirection].filter((d) => d !== oppositeDirection)
      );
    ...
    switch (currentDirection) {
      case 'up':
        nextNodePosition = (Math.ceil(mostRecentCoordinate[1] / squareLength) - 1) * squareLength;
        ...
        return;
      case 'down':
        nextNodePosition = (Math.floor(mostRecentCoordinate[1] / squareLength) + 1) * squareLength;
        nextLinePosition = mostRecentCoordinate[1] + step;
        ...
        return;
      case 'left':
        nextNodePosition = (Math.ceil(mostRecentCoordinate[0] / squareLength) - 1) * squareLength;
        ...
        return;
      case 'right':
        nextNodePosition = (Math.floor(mostRecentCoordinate[0] / squareLength) + 1) * squareLength;
        nextLinePosition = mostRecentCoordinate[0] + step;
        lineDistancePastNextNodePosition = nextLinePosition - nextNodePosition;
        if (lineDistancePastNextNodePosition > 0) {
  ...
  shootLine(): void {
    for (const [line, index] of this.lines.map<[Line, number]>((line, index) => [line, index])) {
      ...
      if (xPos >= this.canvas.width || xPos <= 0 || yPos >= this.canvas.height || yPos <= 0) {
  ...
  register(): void {
    ...
    addEventListener('mousemove', this.registeredCallback);
    this.draw();
  }
